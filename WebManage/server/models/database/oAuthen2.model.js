...
class Oauthen2 extends CommonModel {
  ...
  responseLogin(res, user) {
    ...
    if (manifestid <= TableManifest.NEW_REGISTER) {
      ...
      var sqlMain =
        "SELECT user_id FROM user WHERE delete_flag=0 and id_created=" +
        current_id;
      if (manifestid < TableManifest.ADMIN) {
        sqlMain +=
          " UNION " +
          "SELECT id_member FROM decentralization_access WHERE id_admin=" +
          current_id +
      ...
      knex
        ...
        .then(function (x) {
          ...
          var sqlMain1 =
            "SELECT enterprise_id FROM decentralization_access WHERE deleteflag=0 and id_member=" +
            current_id;
          if (manifestid < TableManifest.ADMIN) {
            sqlMain1 =
              "SELECT enterprise_id FROM decentralization_access WHERE deleteflag=0 and id_admin=" +
              current_id;
          }
          knex
            .raw(sqlMain1)
