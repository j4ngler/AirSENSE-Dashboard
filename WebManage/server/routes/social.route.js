...
router
  .route("/getThreshold")
  .get((req, res) => iotCtrl.getThreshhold(req, res));
...
router
  .route("/getStation")
  .get(isAuthenticated, (req, res) => iotCtrl.getStationServer(req, res));

router
  .route("/getStationHome")
  .get((req, res) => iotCtrl.getStationHome(req, res));
router
  .route("/getdatalimit")
  .post(isAuthenticated, (req, res) =>
    iotCtrl.reportDataStationLimit(req, res)
  );
router.route("/getReportStations").post((req, res) => {
  iotCtrl.getReportStations(req, res);
});
router
  .route("/getAbnormalData")
  .post((req, res) => iotCtrl.getAbnormalData(req, res));

router
  .route("/getTotalPosts")
  .get((req, res) => socialCtrl.getTotalPosts(req, res));
router
  .route("/getBlogsPagination")
  .get((req, res) => socialCtrl.getPostsPagination(req, res));
...
router
  .route("/getCurrentAQI")
  .get((req, res) => iotCtrl.getCurrentAQI(req, res));
router.route("/getAqiData").post((req, res) => iotCtrl.getAqiData(req, res)); //get aqi in recent 24h
router
  .route("/getDataRecent")
  .post((req, res) => iotCtrl.getDataRecent(req, res));

router
  .route("/dataSensor")
  .post((req, res) => iotCtrl.getDataStation(req, res));
